...
STAGE_DAILY_TARGETS = {"ironing": {"kg": 50.0}}
...
async def production_kpi(
    ...
    totals = defaultdict(lambda: {"completed": 0, "active": 0, "duration": 0, "stages": defaultdict(int), "qty": defaultdict(float), "stage_qty": defaultdict(lambda: defaultdict(float)), "active_days": set()})
    ...
    for job in jobs:
        ...
        if job.status == "completed":
            ...
            if job.completed_at and job.started_at:
                ...
                data["duration"] += minutes; total_duration += minutes
    ...
    for uid, data in totals.items():
        ...
        completed = data["completed"]; active_days = len(data["active_days"])
        ...
        for stage, daily_targets in STAGE_DAILY_TARGETS.items():
            target_by_stage[stage] = {}; achievement_by_stage[stage] = {}
            for unit, daily_target in daily_targets.items():
                target_total = round(daily_target * active_days, 2); actual = round(quantity_by_stage.get(stage, {}).get(unit, 0.0), 2)
                target_by_stage[stage][unit] = target_total; achievement_by_stage[stage][unit] = round((actual / target_total) * 100, 1) if target_total else 0.0
        employees.append(ProductionKpiEmployee(
            user_id=uid, employee_name=(user.full_name if user and user.full_name else user.email if user else str(uid)),
            completed_jobs=completed, active_jobs=data["active"], total_duration_minutes=data["duration"],
            average_duration_minutes=round(data["duration"] / completed, 1) if completed else 0.0, active_days=active_days,
            quantity_by_unit={unit: round(qty, 2) for unit, qty in data["qty"].items()},
            by_stage={stage: data["stages"].get(stage, 0) for stage in STAGES}, quantity_by_stage=quantity_by_stage,
            target_by_stage=target_by_stage, achievement_by_stage=achievement_by_stage,
        ))
