...
PRODUCTION_STATUSES = {"washing", "ironing", "folding", "packing"}
...
def _order_out(order: Order) -> OrderOut:
    return OrderOut(
        ...
        invoice_number=order.invoice_number, received_at=order.received_at,
        due_at=order.due_at, status=order.status, subtotal=order.subtotal,
        discount=order.discount, total=order.total, paid_amount=order.paid_amount,
...
async def _load_order(db: AsyncSession, order_id: uuid.UUID) -> Order:
    result = await db.execute(select(Order).options(selectinload(Order.customer), selectinload(Order.items)).where(Order.id == order_id))
...
async def _load_services_for_items(db: AsyncSession, branch_id: uuid.UUID, service_ids: list[uuid.UUID]) -> dict[uuid.UUID, Service]:
    result = await db.execute(select(Service).where(Service.branch_id == branch_id, Service.id.in_(service_ids), Service.is_active.is_(True)))
    services = {s.id: s for s in result.scalars().all()}
    missing = [sid for sid in service_ids if sid not in services]
    if missing:
        raise HTTPException(status_code=400, detail="One or more services are invalid, inactive, or belong to another branch")
    return services


@router.get("", response_model=list[OrderListOut])
async def list_orders(
    branch_id: uuid.UUID | None = None, q: str | None = Query(default=None, max_length=100),
    order_status: str | None = None, payment_status: str | None = None,
    ...
    if branch_id is not None: stmt = stmt.where(Order.branch_id == branch_id)
    if q and q.strip():
        pattern = f"%{q.strip()}%"
        stmt = stmt.where(or_(Order.invoice_number.ilike(pattern), Customer.name.ilike(pattern), Customer.phone.ilike(pattern)))
    if order_status: stmt = stmt.where(Order.status == order_status)
    if payment_status: stmt = stmt.where(Order.payment_status == payment_status)
    stmt = stmt.order_by(Order.received_at.desc())
    result = await db.execute(stmt)
    return [OrderListOut(
        id=o.id, branch_id=o.branch_id, customer_id=o.customer_id, customer_name=o.customer.name,
        invoice_number=o.invoice_number, received_at=o.received_at, due_at=o.due_at, status=o.status,
        subtotal=o.subtotal, discount=o.discount, total=o.total, paid_amount=o.paid_amount,
        payment_status=o.payment_status,
    ) for o in result.scalars().all()]


@router.get("/{order_id}/history", response_model=list[OrderStatusLogOut])
async def get_order_history(order_id: uuid.UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> list[OrderStatusLogOut]:
    order = await _load_order(db, order_id)
    await ensure_branch_accessible(order.branch_id, current_user, db)
    result = await db.execute(select(OrderStatusLog).options(selectinload(OrderStatusLog.changed_by)).where(OrderStatusLog.order_id == order_id).order_by(OrderStatusLog.changed_at.asc()))
    return [OrderStatusLogOut(
        id=log.id, order_id=log.order_id, branch_id=log.branch_id, from_status=log.from_status,
        to_status=log.to_status, changed_by_user_id=log.changed_by_user_id,
        changed_by_name=log.changed_by.full_name if log.changed_by else None,
        changed_at=log.changed_at, note=log.note,
...
async def create_order(payload: OrderCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> OrderOut:
    ...
    if customer is None or customer.branch_id != payload.branch_id or not customer.is_active:
        raise HTTPException(status_code=400, detail="Customer is invalid, inactive, or belongs to another branch")
    ...
    if payload.paid_amount > 0 and not payload.payment_method:
        raise HTTPException(status_code=400, detail="Payment method is required when paid amount is greater than zero")
    ...
    if len(service_ids) != len(set(service_ids)):
        raise HTTPException(status_code=400, detail="Each service may appear only once per order")
    ...
    order = Order(
        branch_id=payload.branch_id, customer_id=customer.id, invoice_number=await _next_invoice(db, payload.branch_id),
        received_at=received_at, due_at=payload.due_at, status="received", subtotal=subtotal,
        discount=payload.discount, total=total, paid_amount=payload.paid_amount,
        payment_status=_payment_status(total, payload.paid_amount), payment_method=payload.payment_method,
        notes=payload.notes, items=item_models,
    )
    db.add(order)
    db.add(OrderStatusLog(order_id=order.id, branch_id=order.branch_id, from_status=None, to_status="received", changed_by_user_id=current_user.id, changed_at=received_at, note="Nota dibuat"))
    db.add(ProductionJob(order_id=order.id, branch_id=order.branch_id, stage="washing", status="pending"))
    await db.commit()
...
async def update_order(order_id: uuid.UUID, payload: OrderUpdate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> OrderOut:
    ...
    if order.status in {"picked_up", "cancelled"} and data and any(field in data for field in {"due_at", "discount", "paid_amount", "payment_method", "notes"}):
    ...
    if "status" in data:
        ...
        if new_status not in ALLOWED_STATUSES:
            raise HTTPException(status_code=400, detail="Invalid order status")
        if new_status != order.status and new_status in PRODUCTION_STATUSES:
            raise HTTPException(status_code=400, detail="Production status must be changed through the production workflow")
        if new_status != order.status and new_status not in STATUS_TRANSITIONS[order.status]:
            raise HTTPException(status_code=400, detail=f"Invalid status transition: {order.status} -> {new_status}")
    ...
    for field, value in data.items():
        if field in {"discount", "paid_amount", "payment_method"}: continue
    ...
    if status_changed:
        db.add(OrderStatusLog(order_id=order.id, branch_id=order.branch_id, from_status=old_status, to_status=order.status, changed_by_user_id=current_user.id, changed_at=datetime.now(timezone.utc), note=None))
    await db.commit()
    return _order_out(await _load_order(db, order.id))
